...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * 一段过程是否**没有善终**：里面有被打断的助手步，或还没结算的工具调用。
     *
     ...
     * - 工具块：调用**还在跑**（`running`，说明回合结束时它没回来），或错误码属于
     *   {@link INTERRUPT_CODES}（「有人把它掐了」）。
     *   ⚠️ **超时与结果未知不算**（用户裁决）：那是运行环境的问题，不该把整块思考标成「被打断」。
     *
    ...
    function cutOffOf(nodes) {
      for (const node of nodes ?? []) {
        ...
        const block = toolBlockOfNode(node)
        if (block === null) continue
        if (toolStatusOf(block) === 'running') return true
        if (isSettledToolBlock(block) && INTERRUPT_CODES.has(toolErrorCodeOf(block))) return true
      }
      return false
    }
    ...
    function buildTurnGroup(turn, inputNode, nodes, subagents, timelineTurn) {
      ...
      /**
       * 回合**没有善终**：结束了但还留着「进行中」的任务，或有调用停在做（`running`）。
       *
      ...
      const unfinished =
        closed &&
        (segments.some((segment) => segment.todos.some((todo) => todo.status === 'in_progress')) ||
...
/* 规划过程：与「任务过程」同级但**长得不一样**（用户要求两者要能区分开）——
   左侧竖线 + 更淡的标题，一眼看出这是「动手之前」的那一段折叠。 */
.dcf-planfold>.dcf-row{border-left:2px solid var(--dsw-alias-border-l2);padding-left:8px}
.dcf-planfold .dcf-title{color:var(--dsw-alias-label-tertiary)}

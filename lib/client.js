...
`
     ...
     * - `turn-tail`：回合尾部控制器。它的收尾正文与最后一个 `assistant-step` 是**同一段文字**，
     *   两边都渲染会让最终答案出现两次。
     */
    const OWNED_NODE_KINDS = new Set(['turn-process', 'turn-tail'])
     ...
     *
     * @param props - `node`、`row`、`seat`（`{owner, renderSlot}`）、`t`、`sessionId`、`labels`。
     * @returns 行。
     */
    function NativeNodeRow({ node, row, seat, t, sessionId, labels }) {
     ...
     *
     * @param props - `blockKey`、`nodes`、`models`、`stats`、`t`、`sessionId`、`active`、`unfinished`、`labels`、`seat`。
     * @returns 折叠块。
     */
    function ThinkingBlock({ blockKey, nodes, models, stats, t, sessionId, active, unfinished, labels, seat }) {

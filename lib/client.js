...
.dcf-plate{background:0 0;border:0;border-radius:0;padding:0}
.dcf-platehead{display:flex;align-items:center;gap:8px;width:100%;min-width:0;background:0 0;border:none;border-radius:6px;padding:4px 6px;font:inherit;color:inherit;text-align:left;cursor:pointer}
.dcf-platehead:hover{background:var(--dsw-alias-interactive-bg-hover)}
.dcf-platetitle{flex:none;color:var(--dsw-alias-label-secondary)}
...
`
     ...
     * @returns 折叠块。
     */
    function ThinkingBlock({ blockKey, entries, stats, t, sessionId, active, unfinished, labels }) {
    ...
    /** 一条直接显示的节点。 */
    function InlineNode({ node, labels, t }) {
      if (node?.kind === 'user' || node?.kind === 'steering') {
        // 传 key 与 kind：用户消息座位要带核心约定的 DOM 属性，别人的插件（如回退按钮）靠它挂载。
        return h(UserBubble, { text: messageTextOf(node), nodeKey: node.key, kind: node.kind, t })
      }
     ...
     */
    function NodeSequence({ nodes, blockKey, t, sessionId, labels, active, unfinished, subagents, mode }) {
      ...
      const children = []
      // mode 只渲染其中一半：把「过程」折进任务阶段、把正文留在外面（任务总结要一直可见）。
      if (entries.length > 0 && mode !== 'inline') {
        children.push(h(ThinkingBlock, { key: 'thinking', blockKey, entries, stats, t, sessionId, active, unfinished, labels }))
      }
      if (mode !== 'process') {
        for (const node of inline) children.push(h(InlineNode, { key: node.key, node, labels, t }))
      }
     ...
     */
    function TaskFold({ segment, t, sessionId, labels, live, unfinished, subagents }) {
    ...
    function TurnGroup({ group, t, sessionId, labels, live }) {
      ...
      // 用户手动点过就以用户的选择为准（只有显式切换才写折叠状态）。
      const stageOpenDefault = live === true
      ...
      if (group.inputText !== '') {
        children.push(
          h(UserBubble, { key: 'ask', text: group.inputText, nodeKey: group.input?.key, kind: 'user', t }),
        )
      }
      ...
      if (group.planned) {
        if (group.planNodes.length > 0) {
          stage.push(h(PlanGroup, { key: 'plan', group, t, sessionId, labels, live }))
        }
        ...
        for (const segment of group.segments) {
          stage.push(
            h(SnapshotPlate, {
              ...
              t,
              sessionId,
              defaultOpen: segment.index === lastIndex && live === true,
              unfinished,
            }),
          )
          const fold = h(TaskFold, { key: `task:${segment.key}`, segment, t, sessionId, labels, live, unfinished, subagents: group.subagents })
          if (fold !== null) stage.push(fold)

...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * - 工具块：`toolStatusOf` 判成 `running` / `cancelled`（客户端把中断合成为 `error.code`）。
     *
    ...
    function cutOffOf(nodes) {
      for (const node of nodes ?? []) {
        ...
        const block = toolBlockOfNode(node)
        if (block === null) continue
        const status = toolStatusOf(block)
        if (status === 'running' || status === 'cancelled') return true
      }
      return false
    }
    ...
    function buildTurnGroup(turn, inputNode, nodes, subagents, timelineTurn) {
      ...
      return {
         ...
         * 本回合**最后一个节点**的 key。
         *
         * 「思考中」只属于「此刻还在往里写的那一块」：把每段过程的 `active` 判据从
         * 「所属回合在跑」收窄到「这一块里包含回合的最后一个节点」，于是一个节点写完之后
         * 它就不再显示「思考中」，而是显示「思考已完成」——回合级的 `live` 只留给阶段折叠。
         */
        liveKey: nodes.length === 0 ? null : nodes[nodes.length - 1].key,
...
`
     ...
     *
     * **默认收起**（用户要求「任务列表默认收起」）：用户需要看时点一下；快照本身仍然按当时状态冻结，
     * 折叠不影响它的语义。首个快照（计划本身）也是收起的，一行摘要就够认出它。
     *
     * @param props - `segment`、`t`、`sessionId`、`unfinished`。
     * @returns 快照面板。
     */
    function SnapshotPlate({ segment, t, sessionId, unfinished }) {
      const [open, toggle] = useCollapse(sessionId, `plate:${segment.key}`, false)

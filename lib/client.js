...
`
      ...
      return h(
        ...
        h(
          'button',
          {
            ...
            'aria-expanded': interactive ? open === true : undefined,
     ...
     * @returns 折叠块。
     */
    function ProcessingBlock({ blockKey, entries, stats, t, sessionId }) {
     ...
     *
     * @param props - `nodes`、`blockKey`、`t`、`sessionId`、`labels`。
     * @returns 节点序列；没有任何可显示内容时返回 `null`。
     */
    function NodeSequence({ nodes, blockKey, t, sessionId, labels }) {
      ...
      const children = []
      if (entries.length > 0) {
        children.push(h(ProcessingBlock, { key: 'process', blockKey, entries, stats, t, sessionId }))
      }
        ...
        h(
          ...
          ...group.plan.todos.map((todo, index) =>
            h(
              ...
              h(
                'button',
                { type: 'button', className: 'dcf-taskrow', 'data-status': todo.status, disabled: true },
